            ...
            <CardContent class="space-y-4">
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div v-if="dish.serving_size">
                  ...
                  <div class="font-medium">{{ dish.serving_size }} {{ dish.serving_unit }}</div>
                </div>
              ...
              <div v-if="dish.image_url" class="mt-4">
                <img 
                  :src="dish.image_url" 
                  :alt="dish.dish_name"
        ...
        <div class="space-y-6">
          ...
          <Card v-if="dish.pricing?.length">
            ...
            <CardContent class="space-y-3">
              <div v-for="pricing in dish.pricing" :key="pricing.pricing_id" class="border rounded-lg p-3">
                <div class="flex items-center justify-between mb-2">
                  <span class="font-medium">{{ getPriceTypeLabel(pricing.price_type) }}</span>
                  <div class="text-lg font-bold">₱{{ pricing.base_price.toFixed(2) }}</div>
                </div>
                
                <div v-if="pricing.promotional_price" class="text-sm text-muted-foreground mb-1">
                  <span class="line-through">₱{{ pricing.base_price.toFixed(2) }}</span>
                  <span class="ml-2 text-red-600 font-medium">₱{{ pricing.promotional_price.toFixed(2) }}</span>
                </div>
                
                <div class="text-xs text-muted-foreground">
                  Min. Margin: {{ pricing.min_profit_margin }}%
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent class="space-y-3">
              <div class="flex justify-between">
                <span class="text-sm">Ingredient Cost:</span>
                <span class="font-medium">₱{{ getTotalIngredientCost().toFixed(2) }}</span>
              </div>
              
              <div v-if="getLatestCost()" class="space-y-2">
                <Separator />
                <div class="text-xs text-muted-foreground">Latest Cost Calculation:</div>
                <div class="flex justify-between text-sm">
                  <span>Labor Cost:</span>
                  <span>₱{{ getLatestCost()!.labor_cost.toFixed(2) }}</span>
                </div>
                <div class="flex justify-between text-sm">
                  <span>Overhead Cost:</span>
                  <span>₱{{ getLatestCost()!.overhead_cost.toFixed(2) }}</span>
                </div>
                <Separator />
                <div class="flex justify-between font-medium">
                  <span>Total Cost:</span>
                  <span>₱{{ getLatestCost()!.total_cost.toFixed(2) }}</span>
                </div>
              </div>

              <div v-if="dish.pricing?.length">
                <Separator />
                <div class="text-xs text-muted-foreground mb-2">Profit Margins:</div>
                <div v-for="pricing in dish.pricing" :key="pricing.pricing_id" class="flex justify-between text-sm">
                  <span>{{ getPriceTypeLabel(pricing.price_type) }}:</span>
                  <span class="text-green-600">
                    {{ ((pricing.base_price - getTotalIngredientCost()) / pricing.base_price * 100).toFixed(1) }}%
                  </span>
                </div>
              </div>
            </CardContent>
